...
class GF256Polynomial {
    /**
     * List of exponents. This is kind of backwards but it can be seen as follows:
     * 
     *     $exponents = array(1, 2, 3, 4);
    ...
    /**
     * The following log and antilog tables are found on this url: http://www.thonky.com/qr-code-tutorial/log-antilog-table/
     * and are calculated by following the steps here: http://www.thonky.com/qr-code-tutorial/error-correction-coding/#step-6-understand-multiplication-with-logs-and-antilogs
     * A possible todo will be to generate this on the fly and not via tables
     * 
     * @var array
     */
    protected $log = array(
        ...
        28, 56, 112, 224, 221, 167, 83, 166, 81, 162, 89, 
        178, 121, 242, 249, 239, 195, 155, 43, 86, 172, 
        69, 138, 9, 18, 36, 72, 144, 61, 122, 244, 
        245, 247, 243, 251, 235, 203, 139, 11, 22, 44, 
        88, 176, 125, 250, 233, 207, 131, 27, 54, 108, 
        216, 173, 71, 142, 1
    );
    
    protected $antilog = array(
        null, 0, 1, 25, 2, 50, 26, 198, 3, 223, 
    ...
    /**
     * Convert exponent to coeffecient
     * 
     * @param int $int
     * @return int
     */
    ...
    /**
     * Convert coefficient to exponent
     * 
     * @param int $int
     * @return int
     */
    public function antilog($int) {
        if (is_null($int)) {
            return 0;
        }
        ...
        return $this->antilog[$int];
    }
    
    /**
     * Return the string of the exponent formula
     * 
     * @return string
     */    
    public function __toString() {
        ...
        $count = count($this->exponents) - 1;
        
        foreach ($this->exponents as $key => $coefficient) {
    ...
    /**
     * Get the number of exponents
     * 
     * @return int
     */
    public function getCount() {
        return count($this->exponents);
    }
    
    /**
     * Multiply the formula by x^$n
     * 
     * @param int $n
     */
    public function multiplyByXn($n) {
        $elements = array_fill(0, $n, null);
        $this->exponents = array_merge($this->exponents, $elements);
    }
    ...
    /**
     * Set all of the exponents
     * 
     * @param array $exponents
     */
    public function setExponents($exponents) {
     ...
     * 
     * @param \QR\GF256Polynomial $divisor
    ...
    public function divideBy(GF256Polynomial $divisor) {
        ...
        do {
            ...
            foreach ($divisorExponents as $divisorKey => $divisorExponent) {
                if (is_null($divisorExponent)) {
                    $messageExponents[$divisorKey] = $this->logXor($messageExponents[$divisorKey], null);
                    continue;
                }
                
                $number = $divisorExponents[$divisorKey] + $messEx;
                
                if ($number > 255) {
                    $number %= 255;
                }                
                
                $messageExponents[$divisorKey] = $this->logXor($messageExponents[$divisorKey], $number);
            }
        ...
        $remainderPolynomial->setExponents($messageExponents);
        return $remainderPolynomial;
    }
    
    /**
     * Convert the exponents to coefficients, add and convert pack to exponent
     * 
     * @param int $a
     * @param int $b
     * @return int
     */
    public function logXor($a, $b) {
        return $this->antilog($this->log($a) ^ $this->log($b));
    }
}